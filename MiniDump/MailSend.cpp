...
void CMailSend::SendMail()
{
	...
	MailHelper::TextPlainContent  textContent(szInfo);
	ofs<<L"textContent"<<endl;

	MailHelper::AppOctStrmContent AppContent(m_strFilePath);
	ofs<<m_strFilePath.c_str()<<endl;

	MailHelper::MailInfo info;
	info.setSenderName(m_strMailUser);
	info.setSenderAddress(m_strMailUser);
	info.setPriority(MailHelper::Priority::normal);
	...
	try
	{
		...
		MailHelper::MailSender sender(m_strMailServer, m_strMailUser, m_strMailPwd);
		ofs<<L"setMail"<<endl;
		sender.setMail(&MailHelper::MailWrapper(&info));
		ofs<<L"sendMail"<<endl;
		sender.sendMail();
		ofs<<L"Send over."<<endl;
	}
	catch (MailHelper::MailException& e)
	{
		cout<<e.error();
		ofs<<e.error();
		throw e.error();
